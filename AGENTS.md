...
Кожен сканер відповідає за **один диск за раз**. Він забирає поточний `drives.json`, виконує upsert запису диска за `name`, і робить PUT назад. Диски, які не сканувалися, залишаються без змін.
...
- Як виставити `GITHUB_TOKEN` на macOS (zsh) і Windows (PowerShell)
- Як запускати кожен сканер
...
Запустити `scan-mac.sh` на маленькій папці. Переконатися що:
- Інтерактивний вибір тома працює
...
- `scannedFrom` — рівно одне зі значень: `"Mac Mini"`, `"MacBook"`, `"PC"`.
...
1. **Перевірити `GITHUB_TOKEN`** — якщо немає, вийти з кодом 1 і повідомленням-інструкцією.
2. **Показати зовнішні томи** з їхніми label, виключивши системний диск.
3. **Інтерактивний вибір**: надрукувати нумерований список, прочитати вибір зі stdin.
4. **Підтвердити назву диска**: запропонувати label тома як значення за замовчуванням, дозволити override.
5. **Підтвердити назву машини**:
   - macOS: запитати один раз (`Mac Mini` чи `MacBook`), зберегти у `~/.twix-drives-machine` для наступних запусків. Підтримати `--reset-machine`, який видаляє файл.
   - Windows: захардкоджено `PC` (єдина Windows-машина).
6. **Просканувати корінь** вибраного тома:
...
7. **Зібрати інформацію про том**: тип файлової системи, total/free/used у байтах.
8. **GET** `data/drives.json` з GitHub. Якщо 404 — почати з порожнього об'єкта (без `sha` у наступному PUT).
9. **Upsert** запису диска за `name`. Виставити `scannedAt` у поточну UTC ISO 8601. Перед збереженням сортувати `entries` за `sizeBytes` за спаданням (менші diff-и JSON). Опційно сортувати `drives` за `name` за зростанням.
10. **Оновити `updatedAt`** на кореневому об'єкті.
11. **PUT** новий вміст (base64) з попереднім `sha` (або без `sha`, якщо це перший раз).
12. **У разі успіху** (200 або 201): надрукувати URL Cloudflare Pages.
13. **У разі помилки мережі / 5xx / 409**: надрукувати новий JSON у stdout (щоб користувач міг скопіювати), вийти з кодом 1.
...
```json
{
  "message": "scan: Black 3 from MacBook at 2026-05-15T14:30:00Z",
  "content": "<base64>",
  "sha": "<previous_sha>",
  "branch": "main"
}
...
- `GITHUB_TOKEN` — fine-grained PAT з правами `Contents: read/write` тільки на цей репо. Обов'язково. **Ніколи не логувати, не друкувати, не вставляти у commit message.** Якщо змінної немає — сканер падає одразу:
  ```bash
...
```
┌─────────────────────────────────────────────────────────┐
│  Накопичувачі                  Оновлено: 15.05.2026     │
│  ┌────────────────────────────────────────────┐         │
...
- **Верхня панель**: заголовок "Накопичувачі", підзаголовок із форматованим `updatedAt`, нижче поле пошуку
- **Сітка**: 1 колонка на mobile, 2 колонки `sm:`, 3 колонки `lg:`
- **Кожна картка**:
  - Назва диска (велика, font-medium)
  - Бейдж `scannedFrom` (малий, приглушений)
...
- **Клік по картці** → відкриває детальний перегляд (slide-over panel із правого боку, ширина ~480 px на desktop, full-screen на mobile). У деталях — повний список entries, відсортований за `sizeBytes` desc, з іконкою типу (📁/📄), назвою, розміром. Hover-стан на рядках.
...
2. Якщо запит порожній → показати всі диски, сортувати за `name` за зростанням, **не** показувати блок збігів.
...
4. **На кожній картці у відфільтрованому стані** під лічильником додати блок:
   ...
   ```
   - Перші до 5 матчів, відсортовані за `sizeBytes` desc.
   - Якщо матчів більше — рядок "+ ще N збігів".
...
| Machine badge | `Сканований з: {machine}` (або просто бейдж із назвою машини) |
...
9. Задати env var на кожній машині:
   - **macOS (zsh):** у `~/.zshrc` додати рядок
     ```bash
     export GITHUB_TOKEN='github_pat_…'
     ```
     потім `source ~/.zshrc` (або перезапустити термінал).
   - **Windows (PowerShell):**
     ```powershell
     [Environment]::SetEnvironmentVariable('GITHUB_TOKEN', 'github_pat_…', 'User')
     ```
     закрити й знову відкрити PowerShell, щоб нова змінна підхопилася.
...
- [ ] `scan-mac.sh` на macOS: список томів без системного диска й без `Macintosh HD`/`Macintosh HD - Data`, інтерактивний вибір працює, PUT повертає 200/201, файл оновлюється
- [ ] `scan-win.ps1` на Windows 10/11: список томів без диска `C:`, працює аналогічно
