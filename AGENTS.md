...
5. **Підтвердити назву машини**:
   - macOS: обрати (`Mac Mini` чи `MacBook`) під час запуску. Опційно підтримувати `TWIX_MACHINE_NAME` для неінтерактивного запуску.
...
9. **Upsert** запису диска за `name`. Виставити `scannedAt` у поточну UTC ISO 8601. Перед збереженням сортувати `entries` у порядку файлового менеджера: спершу папки, потім файли; в межах групи — за назвою. Опційно сортувати `drives` за `name` за зростанням.
