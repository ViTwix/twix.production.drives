...
Кожен запуск сканера проходить **усі підключені несистемні диски** на поточній машині. Після сканування він забирає поточний `drives.json`, виконує upsert записів дисків за `name`, і робить PUT назад.
...
- Автоматичне проходження всіх підключених несистемних томів працює
...
3. **Автопрохід томів**: у межах одного запуску послідовно просканувати кожен знайдений том.
4. **Назва диска**: брати label/назву тому як `name`.
5. **Назва машини не зберігається**: сканер не пише у JSON інформацію про пристрій.
6. **Просканувати корінь** кожного знайденого тому:
